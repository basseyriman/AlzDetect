                      ...
                      <tr key={result.id} className="group hover:bg-slate-50/50 transition-colors">
                        <td className="py-8 px-4">
                          <input
                            ...
                            onChange={(e) => {
                              // Prevent click from bubbling to the row link
                              e.stopPropagation();
                              const newSelected = new Set(selectedResults);
                              if (newSelected.has(result.id)) { newSelected.delete(result.id); } else { newSelected.add(result.id); }
                              setSelectedResults(newSelected);
                            }}
                            className="w-5 h-5 rounded-lg border-slate-200 text-indigo-600 focus:ring-indigo-500 cursor-pointer transition-all pr-print-hidden"
                          />
                        </td>
                        <td className="py-8 px-4">
                          <Link href={`/results/${result.id}`} className="flex flex-col hover:text-indigo-600 transition-colors">
                            <span className="text-sm font-bold text-slate-900">{new Date(result.timestamp).toLocaleDateString()}</span>
                            <span className="text-[10px] font-medium text-slate-400 break-words max-w-[100px]">{result.id.split('-')[0]}</span>
                          </Link>
                        </td>
                        <td className="py-8 px-4">
                          <Link href={`/results/${result.id}`} className="flex items-center gap-3 w-full">
                            ...
                            <span className="text-sm font-black text-slate-600 truncate max-w-[120px] group-hover:text-indigo-600 transition-colors">{result.fileName}</span>
                          </Link>
                        </td>
